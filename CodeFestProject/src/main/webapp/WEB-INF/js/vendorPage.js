...
function createTransactionTable(transactiondetails) {
	...
	if (rowCount > 0) {
		$('#example_info').text(
				'Showing 1 to ' + (rowCount - 1) + ' of 12 entries');
	} else {
...
function createMenuTable(menuDetails) {
	...
	$
			.each(
					...
					function(i, obj) {
						var tableRow = $('<tr id=MmenuTableRow' + iVal + '>');
						var tableData = $('<td>').text(obj.menuId).appendTo(
								tableRow);
						var tableData = $('<td>').text(obj.menuName).appendTo(
								tableRow);
						var tableData = $('<td>').text(obj.menuDescription)
								.appendTo(tableRow);
						var tableData = $('<td>').text(obj.price).appendTo(
								tableRow);
						var tableData = $('<td>').text(obj.quantity).appendTo(
								tableRow);
						var tableData = $('<td>').text(obj.availability)
								.appendTo(tableRow);
						var tableData = $('<td>')
								.append(
